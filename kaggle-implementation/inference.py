...
class RecommendationEngine:
    ...
    def __init__(
        ...
        # This assumes the trainer saved the config dict used for training in the checkpoint
        model_config = checkpoint.get('config')
        if model_config is None:
            # Fallback or error if config not in checkpoint - for demo, we might use demo defaults
            print("Warning: 'config' not found in checkpoint. Using potentially inferred or default model params.")
            # For this demo, let's define a fallback that matches demo.py's training config
            # if it's specifically the demo checkpoint.
            # This is a bit of a hack; robust solution is to always have config in checkpoint.
            if "demo_checkpoints" in model_path: # Heuristic for demo
                print("Assuming demo checkpoint, applying demo-specific fallback config for model init.")
                model_config = {
                    'hidden_size': 128, 'num_heads': 2, 'num_layers': 2, 'seq_length': 20,
                    'dropout': 0.1, 'genre_embed_size': 32, 'user_embed_size': 64,
                    # 'temporal_embed_size' should match hidden_size based on demo.py logic
                    'temporal_embed_size': 128 
                }
            else:
                model_config = {} # Will use model defaults mostly

        # Initialize model using parameters from loaded config and metadata
        self.model = EnhancedBERT4Rec(
            num_items=self.metadata['num_items'],
            num_genres=self.metadata['num_genres'], # This should be the full count from metadata
            num_users=self.metadata['num_users'],
            num_occupations=self.metadata['num_occupations'],
            
            # Get architectural params from the loaded (or fallback) model_config
            hidden_size=model_config.get('hidden_size', 256), # Default from EnhancedBERT4Rec if not in config
            num_heads=model_config.get('num_heads', 4),
            num_layers=model_config.get('num_layers', 4),
            max_seq_len=model_config.get('seq_length', 50), # 'seq_length' is key in demo config
            dropout=model_config.get('dropout', 0.2),
            
            genre_embed_size=model_config.get('genre_embed_size', 32),
            user_embed_size=model_config.get('user_embed_size', 64),
            # Crucially, temporal_embed_size should match hidden_size if direct addition is used
            # and the model was trained that way (as in the demo config)
            temporal_embed_size=model_config.get('temporal_embed_size', model_config.get('hidden_size', 32)),

            # Get these from metadata primarily, as they relate to data vocabularies
            max_year=self.metadata.get('max_year', 2020),
            min_year=self.metadata.get('min_year', 1930),
            max_age=self.metadata.get('max_age_group', 7) + 1 
        )
